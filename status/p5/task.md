...
- [ ] Define NPU Driver API (command queue, memory allocation, sync primitives)
- [ ] Implement DMA runtime routines for Tensor tile management
- [ ] Run full end-to-end model (e.g., MobileNetV2 tile) using Driver API
- [ ] Implement TVM/MLIR backend for NPU target
  - [ ] Graph import from `.tflite` flatbuffer
  - [ ] Operator fusion pass (Conv + BN + ReLU)
  - [ ] Tiling pass (auto-tile to TCDM size)
  - [ ] **✅ Gate: Compiler regression** — compile MobileNetV2, ResNet50, YOLOv8 without errors
- [ ] Implement TFLite Delegate on ARM host
  - [ ] Operator partitioning: NPU-supported ops → NPU subgraph, unsupported → ARM fallback
  - [ ] **✅ Gate: TFLite operator audit** — verify all builtin ops classified and tested
- [ ] **✅ Gate: End-to-end SW smoke test** — run MobileNetV2 INT8 inference through full stack on RTL sim

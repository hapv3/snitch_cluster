...
int main(int argc, char** argv) {
    ...
    write_cmd(tb, cmd_base, 1, OP_COMPUTE_ACT_SILU, 0x10000000, 0, 0x10000200);
    ...
    cout << "[TEST] Running SiLU Size 64..." << endl;
    ...
    // Set 2: Size 128
    for (int i = 0; i < 32; i++) {
        tb.write_dram(0x80004000 + i*4, i + 100); // Activations
        tb.write_dram(0x80006000 + i*4, 0); // Clear Output
    }

    cmd_base = setup_cmd_queue(tb, 0x80000000, 5, 10);
    write_cmd(tb, cmd_base, 0, OP_DMA_READ, 0x80004000, 0x10000000, 128);
    write_cmd(tb, cmd_base, 1, OP_COMPUTE_ACT_SILU, 0x10000000, 0, 0x10000200);
    write_cmd(tb, cmd_base, 2, OP_WAIT_COMPUTE, 0, 0, 0);
    write_cmd(tb, cmd_base, 3, OP_DMA_WRITE, 0x80006000, 0x10000200, 128);
    write_cmd(tb, cmd_base, 4, OP_FINISH, 0, 0, 0);

    cout << "[TEST] Running SiLU Size 128..." << endl;
    if (!run_firmware(tb, "../../../sw/npu_runtime/npu_runtime.bin")) {
        cout << "[TEST] FAILED: Timeout on Size 128!" << endl;
        return 1;
    }

    data_written = false;
    for (int i = 0; i < 32; i++) {
        if (tb.read_dram(0x80006000 + i*4) != 0) {
            data_written = true; break;
        }
    }
    if (!data_written) {
        cout << "[TEST] FAILED: No output data for Size 128" << endl;
        return 1;
    }

    cout << "[TEST] SiLU All Sizes PASSED!" << endl;
    return 0;
}

...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        iss->load_bin("../../npu_cluster/tb/fw/tensorlite_ops.bin");
        // Each cluster gets a cluster ID passed in an initial register (e.g. x10 / a0)
        iss->regs[10] = i; 
        clusters.push_back(iss);
    }

    cout << "[INFO] Started RISC-V Firmware Execution for 4 Clusters (YOLO)" << endl;
    ...
    else cout << "[SUCCESS] YOLO Multi-Cluster Task Complete in " << total_cycles << " cycles!" << endl;
